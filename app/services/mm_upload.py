...
async def validate_and_cache_mm_input(file: UploadFile):
    '''
        Validates inputs and throws an APIException if the validation fails.
        Updates uploaded data cache on validation success.
    '''
    file_data = json.load(file.file)

    if not bool(file_data):
        raise APIException(
            APIResponse(
...
def validate_data(data: dict[str, any]) -> (bool, str):
    '''
        Validates inputs going into MM.
    '''
    try:
